...
public class MonsterMaker {
	...
	public static Monster makeRandomMonster(String name, int level) {
		int monster = randomNumber(0, TOTAL_NUMBER_OF_BREEDS - 1);
		switch (monster) {
			case SCORCHITILE:
				return new Scorchitile(name, level, level * 10, level * 10 + 5);
			case SEEDATOPS:
				return new Seedatops(name, level, level * 10, level + 1);
			case DRAGONFISH:
				return new Dragonfish(name, level, level * 11);
			default:
	...
	public static Monster makeRandomWildMonster(int level) {
		...
		switch (monster) {
			...
			case SEEDATOPS:
				System.out.println("That tree 80 yards away is moving mysteriously...");
				return new Seedatops("Wild Seedatops", level, level * 10, level + 1);
			case DRAGONFISH:
				System.out.println("The water is curiously rippling over there but no stone was thrown...");
				return new Dragonfish("Wild Dragonfish", level, level * 11);
			default:
				return new Scorchitile("Should be unreachable", level, level * 10, level * 10 + 5);
		}
	}
