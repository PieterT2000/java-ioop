...
public class GameWorld {
	...
	private void resolveManageTeam(String input) {
		...
		try {
			if (input.equals("r")) {
				...
				int itemNumber = Integer.parseInt(choice[1]) - 1; // if this is a string, it will throw an exception which means
																													// the monster's
				trainer.useItem(itemNumber, monster);
			}
		} catch (NumberFormatException e) {
			if (monster == UNSELECTED) {
				// monsterInput was wrong
				System.out.println("I didn't quite get that. Please check your inputs.");
			} else {
				// item number couldn't be parsed so interpret this as the new name for the
				// specified monster index
				String newName = choice[1];
				trainer.changeMonsterName(monster, newName);
			}
		} catch (Exception e) {
			System.out.println("I didn't quite get that. Please check your inputs.");
		}
		...
		try {
			...
			switch (choice[0]) {
				case "1":
					...
					break;
				case "2":
					trainer.addMonster(new Seedatops(choice[1], trainer.getTrainerLevel(), trainer.getTrainerLevel() * 12,
							trainer.getTrainerLevel()));
					break;
				case "3":
					trainer.addMonster(new Dragonfish(choice[1], trainer.getTrainerLevel(), trainer.getTrainerLevel() * 12));
					break;
			}
			trainer.addRandomItems(3);
	...
	private void outputStartupChoices() {
		// task 2_1
		...
		System.out.println("1 - A Scorchitile, a brave little fire reptile with very sharp claws");
		System.out.println("2 - A Seedatops, a curious plant that survives by attacking others");
		System.out.println("3 - A DragonFish, a fish that can fight as a dragon");
